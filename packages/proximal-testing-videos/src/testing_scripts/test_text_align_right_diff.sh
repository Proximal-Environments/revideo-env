...
# Renders the right-aligned text scene on two branches and asserts they differ.
# Usage: test_text_align_right_diff.sh [base_branch] [solution_branch]

BASE_BRANCH=${1:-text-align-2d-text-base}
SOLUTION_BRANCH=${2:-text-align-2d-text-solution}

SCENE=packages/proximal-testing-videos/src/text_align_right.tsx
OUTDIR=.revideo-tests/text-align-right
mkdir -p "$OUTDIR"
...
echo "PASS: Videos differ as expected (right alignment)."
