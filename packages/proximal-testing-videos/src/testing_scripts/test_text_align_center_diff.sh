...
# Renders the center-aligned text scene on two branches and asserts they differ.
# Usage: test_text_align_center_diff.sh [base_branch] [solution_branch]

BASE_BRANCH=${1:-text-align-2d-text-base}
SOLUTION_BRANCH=${2:-text-align-2d-text-solution}

SCENE=packages/proximal-testing-videos/src/text_align_center.tsx
OUTDIR=.revideo-tests/text-align-center
mkdir -p "$OUTDIR"
...
echo "PASS: Videos differ as expected (center alignment)."
