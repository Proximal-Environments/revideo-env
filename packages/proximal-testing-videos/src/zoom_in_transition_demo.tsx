import {makeProject, Vector2, waitFor, BBox, zoomInTransition} from '@revideo/core';
...
const second = makeScene2D('second', function* (view) {
  ...
  // Focus a central rectangle area when transitioning in
  const area = new BBox(120, 90, 200, 150);
  yield* zoomInTransition(area, 0.8);
  yield* waitFor(1);
});

export const project = makeProject({
  name: 'zoom-in-transition-demo',
  scenes: [first, second],
  settings: {
    shared: {
      background: '#000000',
